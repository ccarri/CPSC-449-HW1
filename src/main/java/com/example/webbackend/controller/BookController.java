...
public class BookController {
    ...
    public List<Book> getBooksPaginated(@RequestParam(defaultValue = "0") int page,
                                        @RequestParam(defaultValue = "5") int size) {

        return books.stream().skip((long) page * size).limit(size).collect(Collectors.toList());
    }
    ...
    public List<Book> getBooksAdvanced(
        ...
        return filtered.stream().skip((long) page * size).limit(size).collect(Collectors.toList());
    }
}